...
def main():
    ...
    stats = {
        ...
        "skipped_closed": 0,
        "invalid": 0,
        "skipped_existing": 0,  # НОВОЕ: пропущено существующих объявлений (--only-new)
    }
    ...
    try:
        for ad_data in iterate_ads(
            ...
            max_time=args.max_time,      # НОВЫЙ ПАРАМЕТР
            only_new=args.only_new,      # НОВЫЙ ПАРАМЕТР
            conn=conn,                   # НОВЫЙ ПАРАМЕТР (для проверки existing)
            start_time=start_time,       # НОВЫЙ ПАРАМЕТР
        ):
